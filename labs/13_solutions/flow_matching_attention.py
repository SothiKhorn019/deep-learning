...
class ResidualBlock(torch.nn.Module):
    ...
    def __init__(self, width: int) -> None:
        ...
        torch.nn.init.zeros_(self.gn2.bias)
...
class SelfAttentionBlock(torch.nn.Module):
    def __init__(self, channels: int, heads: int) -> None:
        ...
        self.heads = heads
        self.gn = torch.nn.GroupNorm(min(channels // 4, 16), channels)
        self.mha = torch.nn.MultiheadAttention(channels, heads, batch_first=True)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        hidden = attn_output.permute(0, 2, 1).view(B, C, H, W)
...
class DownscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int, attention_heads: int | None = None) -> None:
        ...
        if attention_heads is not None:
            self.attention_block = SelfAttentionBlock(width, attention_heads)
...
class UNet(torch.nn.Module):
    ...
    def __init__(self, channels: int, stage_blocks: int, stages: int, attention_stages: int, attention_heads: int):
        ...
        for i in range(stages):
            heads = attention_heads if i >= stages - attention_stages else None
            self.downscaling_blocks.append(DownscalingBlock(stage_blocks, current_channels, heads))
            current_channels <<= 1
        ...
        self.middle_attention = SelfAttentionBlock(middle_channels, attention_heads)

        self.upscaling_blocks = torch.nn.ModuleList()
        current_channels = middle_channels
        for i in range(stages):
            heads = attention_heads if i < attention_stages else None
            output_channels = current_channels >> 1
            self.upscaling_blocks.append(UpscalingBlock(stage_blocks, output_channels, heads))
            current_channels >>= 1
...
class FlowMatching(npfl138.TrainableModule):
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        predicted_vector_field = self._model(x_t, times)
    ...
    def generate(self, initial_noise: torch.Tensor, steps: int) -> torch.Tensor:
        ...
        for i in range(steps):
            ...
            current_t_tensor = torch.full((current_images.shape[0], 1), current_t_scalar, device=self.device)
        ...
        return self.denormalize_image(final_generated_images), list(map(self.denormalize_image, trajectory))
