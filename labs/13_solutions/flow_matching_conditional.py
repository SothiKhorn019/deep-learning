...
class DownscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int) -> None:
        ...
        self.res_blocks = torch.nn.ModuleList([ResidualBlock(width) for _ in range(residual_blocks)])
...
class UNet(torch.nn.Module):
    ...
    def __init__(self, channels: int, stage_blocks: int, stages: int) -> None:
        ...
        middle_channels = channels << stages
    ...
    def forward(self, images: torch.Tensor, conditioning: torch.Tensor, times: torch.Tensor) -> torch.Tensor:
        ...
        upsampled_conditioning = F.interpolate(conditioning, size=images.shape[2:], mode='nearest')
        x = torch.cat([images, upsampled_conditioning], dim=1)
        ...
        skip_connections_list = skip_connections_list[::-1]
...
class FlowMatching(npfl138.TrainableModule):
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        normalized_images_x1 = self.normalize_image(images)
        conditioning = F.avg_pool2d(normalized_images_x1, kernel_size=self._downscale)

        times_bc = times.unsqueeze(-1).unsqueeze(-1)
        ...
        predicted_vector_field = self._model(x_t, conditioning, times.squeeze())
    ...
    @torch.no_grad()
    def generate(self, initial_noise: torch.Tensor, conditioning: torch.Tensor, steps: int) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        normalized_conditioning = self.normalize_image(conditioning)
        current_images = images
        dt = 1.0 / steps

        for i in range(steps):
            trajectory.append(current_images.clone())
            current_t_scalar = i * dt
            current_t_tensor = torch.full((current_images.shape[0],), current_t_scalar, device=self.device)

            predicted_vector_field = self._ema_model(current_images, normalized_conditioning, current_t_tensor)
...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    conditioning = F.avg_pool2d(dev, kernel_size=args.downscale)
    ...
    class TBSampler:
        ...
        def __call__(self, model, epoch, logs) -> None:
            ...
            if epoch == args.epochs or (args.plot_each and epoch % args.plot_each == 0):
                ...
                images = torch.cat([line for lines in zip(
                    [torch.cat(list(F.interpolate(self._c[r], scale_factor=args.downscale)), -1) for r in rows],
