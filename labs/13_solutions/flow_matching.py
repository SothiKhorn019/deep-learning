...
class SinusoidalEmbedding(torch.nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        half_dim = self.dim // 2
        i = torch.arange(half_dim, device=inputs.device, dtype=inputs.dtype)
        denominator = 20 ** (2 * i / self.dim)
        arg = 2 * torch.pi * inputs / denominator

        sin_embeddings = torch.sin(arg)
        cos_embeddings = torch.cos(arg)
...
class ResidualBlock(torch.nn.Module):
    ...
    def __init__(self, width: int) -> None:
        ...
        self.conv1 = torch.nn.LazyConv2d(width, kernel_size=3, padding="same", bias=False)
        self.gn1 = torch.nn.GroupNorm(min(width // 4, 16), width)
        self.swish1 = torch.nn.SiLU() # Swish activation

        self.time_linear = torch.nn.LazyLinear(width)
        self.swish_time = torch.nn.SiLU()

        self.conv2 = torch.nn.LazyConv2d(width, kernel_size=3, padding="same", bias=False)
        self.gn2 = torch.nn.GroupNorm(min(width // 4, 16), width)
        
        # Initialize the weight parameter of the last GroupNorm to all zeros
        # if hasattr(self.gn2, 'weight') and self.gn2.weight is not None: # Check if weight is initialized (not for Lazy)
        #      torch.nn.init.zeros_(self.gn2.weight)
        if self.gn2.weight is not None:
            torch.nn.init.zeros_(self.gn2.weight)
    ...
    def forward(self, images: torch.Tensor, times: torch.Tensor) -> torch.Tensor:
        ...
        time_emb = self.swish_time(self.time_linear(times))
        # Add time embedding to convolutional features.
        # Time embedding shape: [batch_size, width]
        # x shape: [batch_size, width, H, W]
        # We need to reshape time_emb to [batch_size, width, 1, 1] for broadcasting.
...
class DownscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int) -> None:
        ...
        self.down_conv = torch.nn.LazyConv2d(width << 1, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, images: torch.Tensor, times: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for block in self.res_blocks:
            x = block(x, times)
        skip_connection = x # Output of the last residual block
        downscaled_features = self.down_conv(x)
        return downscaled_features, skip_connection
...
class UpscalingBlock(torch.nn.Module):
    ...
    def __init__(self, residual_blocks: int, width: int) -> None:
        ...
        self.up_conv_transpose = torch.nn.LazyConvTranspose2d(width, kernel_size=4, stride=2, padding=1)
        self.skip_conv = torch.nn.LazyConv2d(width, kernel_size=3, padding="same")
    ...
    def forward(self, images: torch.Tensor, skip_connections: torch.Tensor, times: torch.Tensor) -> torch.Tensor:
        ...
        x = self.up_conv_transpose(images)
        processed_skip = self.skip_conv(skip_connections)
        x = x + processed_skip
...
class UNet(torch.nn.Module):
    ...
    def __init__(self, channels: int, stage_blocks: int, stages: int) -> None:
        ...
        self.initial_conv = torch.nn.LazyConv2d(channels, kernel_size=3, padding="same")

        self.downscaling_blocks = torch.nn.ModuleList()
        current_channels = channels
        for _ in range(stages):
            self.downscaling_blocks.append(DownscalingBlock(stage_blocks, current_channels))
            current_channels <<= 1 # Double channels for the next downscaling block's *output*

        self.middle_blocks = torch.nn.ModuleList()
        middle_channels = channels << stages # Channels after last downscaling block output
        for _ in range(stage_blocks):
            self.middle_blocks.append(ResidualBlock(middle_channels))

        self.upscaling_blocks = torch.nn.ModuleList()
        for i in range(stages):
            # The input to upscaling block is `middle_channels` (or output of previous upscaling)
            # The output width of upscaling block should be `middle_channels >> 1`
            # The skip connection comes from downscaling block with `middle_channels >> 1` channels
            upscaling_output_channels = middle_channels >> 1
            self.upscaling_blocks.append(UpscalingBlock(stage_blocks, upscaling_output_channels))
            middle_channels >>=1


        self.output_conv = torch.nn.LazyConv2d(Image64Dataset.C, kernel_size=3, padding="same")

    def forward(self, images: torch.Tensor, times: torch.Tensor) -> None:
        # TODO: Implement the forward pass of the U-Net.
        if times.ndim == 1:
            times_for_embedding = times.unsqueeze(-1)
        else:
            times_for_embedding = times

        time_emb = self.time_embedding(times_for_embedding)

        x = self.initial_conv(images)

        skip_connections_list = []
        for block in self.downscaling_blocks:
            x, skip_connection = block(x, time_emb)
            skip_connections_list.append(skip_connection)

        for block in self.middle_blocks:
            x = block(x, time_emb)

        skip_connections_list = skip_connections_list[::-1] # Reverse for upscaling

        for i, block in enumerate(self.upscaling_blocks):
            skip = skip_connections_list[i]
...
class FlowMatching(npfl138.TrainableModule):
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        times = torch.rand(images.shape[0], 1, 1, 1, device=images.device)
        ...
        normalized_images_x1 = self.normalize_image(images) # x_1 (normalized)
        
        # Reshape times to be [batch_size, 1, 1, 1] for broadcasting with images
        if times.ndim == 2: # [batch_size, 1]
            times_bc = times.unsqueeze(-1).unsqueeze(-1) # [batch_size, 1, 1, 1]
        else: # [batch_size, 1, 1, 1]
            times_bc = times
        
        term_x0_coeff = 1 - (1 - self._sigma_min) * times_bc
        x_t = times_bc * normalized_images_x1 + term_x0_coeff * noises
        
        target_vector_field = normalized_images_x1 - (1 - self._sigma_min) * noises

        predicted_vector_field = self._model(x_t, times.squeeze()) # Squeeze times back to [B,1] or [B] for model

        loss = self.loss(predicted_vector_field, target_vector_field)
    ...
    def generate(self, initial_noise: torch.Tensor, steps: int) -> torch.Tensor:
        ...
        current_images = images # This is x_0 (noise)
        dt = 1.0 / steps

        for i in range(steps):
            trajectory.append(current_images.clone()) # Store x_t before update
            current_t_scalar = i * dt
            # Model expects time as a tensor, possibly [batch_size, 1] or [batch_size]
            current_t_tensor = torch.full((current_images.shape[0],), current_t_scalar, device=self.device)

            # Get the predicted vector field v_theta(x_t, t) from the EMA model
            predicted_vector_field = self._ema_model(current_images, current_t_tensor)

            # Euler step: x_{t+dt} = x_t + dt * v(x_t, t)
            current_images = current_images + dt * predicted_vector_field


        final_generated_images = current_images
        
        # Apply the denormalization to the generated images and the trajectory.
        return self.denormalize_image(final_generated_images), [self.denormalize_image(img) for img in trajectory]
