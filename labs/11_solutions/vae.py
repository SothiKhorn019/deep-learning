...
class VAE(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        enc_layers = []
        enc_layers.append(torch.nn.Flatten())
        prev_dim = input_dim
        for h in args.encoder_layers:
            enc_layers.append(torch.nn.Linear(prev_dim, h))
            enc_layers.append(torch.nn.ReLU())
            prev_dim = h
        enc_layers.append(torch.nn.Linear(prev_dim, 2 * args.z_dim))
        ...
        dec_layers = []
        prev_dim = args.z_dim
        for h in args.decoder_layers:
            dec_layers.append(torch.nn.Linear(prev_dim, h))
            dec_layers.append(torch.nn.ReLU())
            prev_dim = h
        dec_layers.append(torch.nn.Linear(prev_dim, MNIST.C * MNIST.H * MNIST.W))
        dec_layers.append(torch.nn.Sigmoid())
        dec_layers.append(torch.nn.Unflatten(1, (MNIST.C, MNIST.H, MNIST.W)))
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        z_mean, z_log_sd = torch.chunk(enc_out, 2, dim=1)
        ...
        reconstruction_loss = F.binary_cross_entropy(recon, images, reduction='mean')
