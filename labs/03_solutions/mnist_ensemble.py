...
def main(args: argparse.Namespace) -> tuple[list[float], list[float]]:
    ...
    for model in range(args.models):
        ...
        for m in range(model + 1):
            model_preds = models[m].predict(dev, data_with_labels=True)  # Only predictions are returned
            model_preds = torch.tensor(np.array(model_preds), dtype=torch.float32)
            model_preds = torch.nn.functional.softmax(model_preds, dim=1)

            all_predictions.append(model_preds)

        # Convert predictions to a tensor and average them
        ...
        labels = torch.cat(all_labels)  # Merge all label batches into one tensor
        
        # Compute final predictions by taking the class with the highest probability
        ensemble_preds = torch.argmax(ensemble_predictions, dim=1)
        
        # Compute accuracy
        accuracy_metric = torchmetrics.Accuracy("multiclass", num_classes=MNIST.LABELS)
        ensemble_accuracy = accuracy_metric(ensemble_preds, labels).item()
