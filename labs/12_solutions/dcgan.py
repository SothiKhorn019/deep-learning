...
class GAN(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        H, W, C = MNIST.H, MNIST.W, MNIST.C
        self.generator = torch.nn.Sequential(
            torch.nn.Linear(self._z_dim, 1024, bias=False),
            torch.nn.BatchNorm1d(1024, eps=0.01),
            torch.nn.ReLU(),
            torch.nn.Linear(1024, 64 * (H // 4) * (W // 4), bias=False),
            torch.nn.BatchNorm1d(64 * (H // 4) * (W // 4), eps=0.01),
            torch.nn.ReLU(),
            torch.nn.Unflatten(1, (64, H // 4, W // 4)),
            torch.nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1, bias=False),
            torch.nn.BatchNorm2d(32, eps=0.01),
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(32, C, kernel_size=4, stride=2, padding=1),
            torch.nn.Sigmoid()
        )
        ...
        self.discriminator = torch.nn.Sequential(
            torch.nn.Conv2d(C, 32, kernel_size=5, padding=2, bias=False),
            torch.nn.BatchNorm2d(32, eps=0.01),
            torch.nn.ReLU(),
            ...
            torch.nn.MaxPool2d(kernel_size=2, stride=2),
            torch.nn.Flatten(),
            torch.nn.Linear(64 * (H // 4) * (W // 4), 1024, bias=False),
            torch.nn.BatchNorm1d(1024, eps=0.01),
            torch.nn.ReLU(),
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        generator_loss = self.loss(preds_fake, real_labels)
        ...
        preds_real = self.discriminator(images)
        preds_fake_detached = self.discriminator(generated.detach())
        fake_labels = torch.zeros(batch_size, 1, device=self.device)
        discriminator_loss = (
            self.loss(preds_real, real_labels) + self.loss(preds_fake_detached, fake_labels)
        )
        ...
        self.metrics["discriminator_accuracy"].update(preds_real, real_labels)
        self.metrics["discriminator_accuracy"].update(preds_fake_detached, fake_labels)
