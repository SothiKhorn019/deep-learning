...
class GAN(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        for units in args.generator_layers:
            gen_layers.append(torch.nn.Linear(input_dim, units))
            gen_layers.append(torch.nn.ReLU())
            input_dim = units
        gen_layers.append(torch.nn.Linear(input_dim, MNIST.C * MNIST.H * MNIST.W))
        gen_layers.append(torch.nn.Sigmoid())
        gen_layers.append(torch.nn.Unflatten(1, (MNIST.C, MNIST.H, MNIST.W)))
        ...
        disc_layers = []
        disc_layers.append(torch.nn.Flatten())
        input_dim = MNIST.C * MNIST.H * MNIST.W
        for units in args.discriminator_layers:
            disc_layers.append(torch.nn.Linear(input_dim, units))
            disc_layers.append(torch.nn.ReLU())
            input_dim = units
        disc_layers.append(torch.nn.Linear(input_dim, 1))
        disc_layers.append(torch.nn.Sigmoid())
    ...
    def train_step(self, xs: tuple[torch.Tensor], y: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        z = self._z_prior().sample([batch_size])
        generated = self.generator(z)
        discriminated_fake_for_g = self.discriminator(generated)
        g_labels = torch.ones_like(discriminated_fake_for_g)
        generator_loss = self.loss(discriminated_fake_for_g, g_labels)
        ...
        fake_detached = generated.detach()
        discriminated_fake = self.discriminator(fake_detached)
        real_labels = torch.ones_like(discriminated_real)
        fake_labels = torch.zeros_like(discriminated_fake)
        d_loss_real = self.loss(discriminated_real, real_labels)
        d_loss_fake = self.loss(discriminated_fake, fake_labels)
        ...
        self.metrics["discriminator_accuracy"].update(discriminated_real, real_labels)
        self.metrics["discriminator_accuracy"].update(discriminated_fake, fake_labels)
...
def main(args: argparse.Namespace) -> dict[str, float]:
    ...
    model.configure(
        ...
        metrics={"discriminator_accuracy": torchmetrics.Accuracy(task="binary")},
        logdir=args.logdir,
    )
