...
class Model(npfl138.TrainableModule):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if args.hidden_layer > 0:
            self.hidden = nn.Linear(args.rnn_dim, args.hidden_layer)

        # Output layer
        output_input_dim = args.hidden_layer if args.hidden_layer > 0 else args.rnn_dim
        self.output = nn.Linear(output_input_dim, 1)

    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        # TODO: Process the input sequence through the RNN and the other layers.
        rnn_output, _ = self.rnn(inputs)  # (batch, seq_len, rnn_dim)

        if self.args.hidden_layer > 0:
            rnn_output = torch.relu(self.hidden(rnn_output))
